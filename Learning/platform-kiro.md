# Learning Agent — Kiro CLI Setup Guide

This guide walks you through deploying the Learning Agent on Kiro CLI. The core prompt lives in `learning-agent-prompt.md` — this file only covers platform-specific setup.
...
3. Create a new file at `.kiro/steering/learning-agent.md`.
...
6. Open a Kiro CLI chat session — the Learning Agent persona is now active.

> To disable, remove or rename the `.kiro/steering/learning-agent.md` file.
...
- The conversational state markers (`[LAYER X/N]`, `[CONCEPT MAP]`, `[KNOWLEDGE CHECK]`, etc.) display as plain text, which works well in a terminal context.
- Keep code block examples concise — long code blocks are harder to read in a terminal without scrollback.

### Limitations

- **Context window**: Long learning sessions may approach the model's context limit. If responses start losing coherence or referencing earlier layers incorrectly, ask for a `[SUMMARY]` and start a new session with the recap.
...
This setup guide does not modify the Learning Agent's teaching behavior, persona, methodology, or interaction patterns. All pedagogical logic lives in the core prompt (`learning-agent-prompt.md`). This file only provides deployment instructions for the Kiro CLI platform.
