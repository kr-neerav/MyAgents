# Blog Evaluator Agent — Kiro CLI Setup Guide

This guide walks you through deploying the Blog Evaluator Agent on Kiro CLI. The core prompt lives in `blog-evaluator-agent-prompt.md` — this file only covers platform-specific setup.
...
6. Open a Kiro CLI chat session — the Blog Evaluator Agent persona is now active.
...
This setup guide does not modify the Blog Evaluator Agent's writing behavior, persona, methodology, or interaction patterns. All writing logic lives in the core prompt (`blog-evaluator-agent-prompt.md`). This file only provides deployment instructions for the Kiro CLI platform.
