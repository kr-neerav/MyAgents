# Proposal Agent — Claude Setup Guide

This guide walks you through deploying the Proposal Agent on Claude. The core prompt lives in `proposal-agent-prompt.md` — this file only covers platform-specific setup.
...
4. Wait for Claude to acknowledge, then begin your proposal drafting session.
...
- Claude has strong instruction-following capabilities. The structured markers (`[PROBLEM FRAMING]`, `[CONTEXT]`, `[SOLUTION]`, `[ALTERNATIVES]`, `[IMPACT]`, `[PROPOSAL DOCUMENT]`, `[SUMMARY]`) and the six-phase proposal drafting methodology in the core prompt are reliably followed.
- The persona, Socratic questioning, and domain-adaptive proposal guidance work well without additional reinforcement.
...
- **Context window**: Long proposal drafting sessions may approach Claude's context limit. If responses start losing coherence or referencing earlier decisions incorrectly, ask for a `[SUMMARY]` and start a new conversation with the recap.
...
This setup guide does not modify the Proposal Agent's proposal drafting behavior, persona, methodology, or interaction patterns. All proposal logic lives in the core prompt (`proposal-agent-prompt.md`). This file only provides deployment instructions for the Claude platform.
