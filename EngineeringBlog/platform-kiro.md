# Engineering Blog Agent — Kiro CLI Setup Guide

This guide walks you through deploying the Engineering Blog Agent on Kiro CLI. The core prompt lives in `engineering-blog-agent-prompt.md` — this file only covers platform-specific setup.
...
2. Open `engineering-blog-agent-prompt.md` and copy the entire contents.
...
6. Open a Kiro CLI chat session — the Engineering Blog Agent persona is now active.
...
- **Context window**: Long writing sessions may approach the model's context limit. If responses start losing coherence or referencing earlier situations incorrectly, ask for a `[SUMMARY]` and start a new session with the recap.
...
This setup guide does not modify the Engineering Blog Agent's writing behavior, persona, methodology, or interaction patterns. All writing logic lives in the core prompt (`engineering-blog-agent-prompt.md`). This file only provides deployment instructions for the Kiro CLI platform.
