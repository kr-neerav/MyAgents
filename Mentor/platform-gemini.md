# Mentor Agent — Gemini Chat Setup Guide

This guide walks you through deploying the Mentor Agent on Google Gemini. The core prompt lives in `mentor-agent-prompt.md` — this file only covers platform-specific setup.
...
3. Open `mentor-agent-prompt.md` and copy the entire contents.
4. Paste the full prompt into the custom instructions text field.
5. Save your changes.
6. Start a new chat — the Mentor Agent persona is now active for all conversations.
...
6. Click **Save**.
7. To start a session, select your "Mentor Agent" Gem from the sidebar.

> Gems keep the Mentor Agent isolated from your regular Gemini conversations.
...
This setup guide does not modify the Mentor Agent's mentoring behavior, persona, methodology, or interaction patterns. All mentoring logic lives in the core prompt (`mentor-agent-prompt.md`). This file only provides deployment instructions for the Gemini platform.
