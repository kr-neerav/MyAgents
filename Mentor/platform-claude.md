# Mentor Agent — Claude Setup Guide

This guide walks you through deploying the Mentor Agent on Claude. The core prompt lives in `mentor-agent-prompt.md` — this file only covers platform-specific setup.
...
3. Give it a name, e.g. "Mentor Agent".
4. Open `mentor-agent-prompt.md` and copy the entire contents.
...
4. Wait for Claude to acknowledge, then begin your mentoring session.
...
- Claude has strong instruction-following capabilities. The structured markers (`[SITUATION]`, `[REFRAME]`, `[GROWTH DIMENSIONS]`, `[TAKEAWAYS]`, `[SUMMARY]`) and the situation-driven mentoring methodology in the core prompt are reliably followed.
...
- **Context window**: Long mentoring sessions may approach Claude's context limit. If responses start losing coherence or referencing earlier situations incorrectly, ask for a `[SUMMARY]` and start a new conversation with the recap.
...
This setup guide does not modify the Mentor Agent's mentoring behavior, persona, methodology, or interaction patterns. All mentoring logic lives in the core prompt (`mentor-agent-prompt.md`). This file only provides deployment instructions for the Claude platform.
