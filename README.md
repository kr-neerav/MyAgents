...
## [Status Update Agent](StatusUpdates/status-update-agent-prompt.md)

A system prompt that turns an LLM into an opinionated writing partner for org status updates. Accepts raw project information in any form — bullet points, Slack messages, meeting notes, conversational descriptions — and produces polished status updates in three formats: Weekly Status Report (Kurt Kufeld VP format with status color, executive summary, and structured sections), Quick Update (2-5 sentences adapted for executive, technical, or cross-functional audiences), and Sprint Summary (goal outcome, completed work, carried-over items, blockers, next goals). Enforces company writing standards — active voice, weasel word elimination, specific metrics, named owners, recommendations with every problem — and supports iterative refinement, audience switching, format conversion, and draft review. Platform setup guides for [Gemini](StatusUpdates/platform-gemini.md), [Kiro CLI](StatusUpdates/platform-kiro.md), and [Claude](StatusUpdates/platform-claude.md).
...
## [SDD Agent](SDD/sdd-agent-prompt.md)

A system prompt that turns an LLM into a Spec Driven Development companion. Guides users through the full spec lifecycle — Requirements → Design → Tasks — replicating Kiro's structured, iterative, phase-based methodology as a conversational experience. Generates requirements using EARS patterns with INCOSE quality rules, user stories, and acceptance criteria with correctness property suggestions; produces design documents with architecture, component decomposition, interfaces, data models, and tradeoff analysis; and breaks designs into ordered implementation tasks with sub-tasks, dependencies, and traceability. Supports a parallel bugfix workflow for bug condition identification, root cause analysis, fix design, and verification tasks. Includes session summary generation for context restoration across sessions. Platform setup guides for [Gemini](SDD/platform-gemini.md), [Kiro CLI](SDD/platform-kiro.md), and [Claude](SDD/platform-claude.md).
