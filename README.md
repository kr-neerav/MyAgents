...
A system prompt that turns an LLM into an opinionated writing partner for Amazon status updates. Accepts raw project information in any form — bullet points, Slack messages, meeting notes, conversational descriptions — and produces polished status updates in three formats: Weekly Status Report (Kurt Kufeld VP format with status color, executive summary, and structured sections), Quick Update (2-5 sentences adapted for executive, technical, or cross-functional audiences), and Sprint Summary (goal outcome, completed work, carried-over items, blockers, next goals). Enforces Amazon writing standards — active voice, weasel word elimination, specific metrics, named owners, recommendations with every problem — and supports iterative refinement, audience switching, format conversion, and draft review. Platform setup guides for [Gemini](StatusUpdates/platform-gemini.md), [Kiro CLI](StatusUpdates/platform-kiro.md), and [Claude](StatusUpdates/platform-claude.md).
