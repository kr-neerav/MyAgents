# Requirements Agent — Claude Setup Guide

This guide walks you through deploying the Requirements Agent on Claude. The core prompt lives in `requirements-agent-prompt.md` — this file only covers platform-specific setup.
...
3. Give it a name, e.g. "Requirements Agent".
4. Open `requirements-agent-prompt.md` and copy the entire contents.
...
4. Wait for Claude to acknowledge, then begin your requirements session.
...
- Claude has strong instruction-following capabilities. The structured markers (`[PROBLEM SCOPE]`, `[ELICIT]`, `[ORGANIZE]`, `[REFINE]`, `[DOCUMENT]`, `[SUMMARY]`) and the requirements engineering methodology in the core prompt are reliably followed.
- The persona, Socratic questioning, and domain-adaptive elicitation patterns work well without additional reinforcement.

### Formatting Support

- Claude renders full markdown: headers, bold, italic, numbered lists, bullet lists, and fenced code blocks all work correctly.
- The conversational state markers (`[PROBLEM SCOPE]`, `[ELICIT]`, `[ORGANIZE]`, `[REFINE]`, `[DOCUMENT]`, `[SUMMARY]`) display as plain text, which is the intended behavior.
- Mermaid diagrams and LaTeX are not used in the core prompt, keeping output portable across contexts.

### Limitations

- **Context window**: Long requirements sessions may approach Claude's context limit. If responses start losing coherence or referencing earlier requirements incorrectly, ask for a `[SUMMARY]` and start a new conversation with the recap.
...
This setup guide does not modify the Requirements Agent's requirements engineering behavior, persona, methodology, or interaction patterns. All requirements engineering logic lives in the core prompt (`requirements-agent-prompt.md`). This file only provides deployment instructions for the Claude platform.
