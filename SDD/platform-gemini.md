# SDD Agent — Gemini Chat Setup Guide

This guide walks you through deploying the SDD Agent on Google Gemini. The core prompt lives in `sdd-agent-prompt.md` — this file only covers platform-specific setup.
...
3. Open `sdd-agent-prompt.md` and copy the entire contents.
4. Paste the full prompt into the custom instructions text field.
5. Save your changes.
6. Start a new chat — the SDD Agent persona is now active for all conversations.
...
6. Click **Save**.
7. To start a session, select your "SDD Agent" Gem from the sidebar.

> Gems keep the SDD Agent isolated from your regular Gemini conversations.
...
- **Context window**: Long spec sessions with multiple phases of refinement may approach Gemini's context limit. If responses start losing coherence or referencing earlier phase decisions incorrectly, ask for a session summary and start a new chat with the recap.
...
This setup guide does not modify the SDD Agent's behavior, methodology, output formatting, or interaction patterns. All agent logic lives in the core prompt (`sdd-agent-prompt.md`). This file only provides deployment instructions for the Gemini platform.
